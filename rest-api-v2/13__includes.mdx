...
<Alert variant="info">
<Alert.Heading>"include[]" is a multi-value parameter</Alert.Heading>
...
Note how including `services` causes a full `Service` <Link to="/docs/rest-api-v2/resource-schemas/">schema</Link> (with type `service`) to be displayed in the `service` field of the incident, while without the `include[]` the `service` field contains a `ServiceReference` <Link to="/docs/rest-api-v2/resource-schemas/">schema</Link> (with type `service_reference`).

<Tabs defaultActiveKey="without" variant="tabs" id="includes-samples">
  <Tab eventKey="without" title="without include[] values">
...
```
  ...
  <Tab eventKey="with" title="with included services">
