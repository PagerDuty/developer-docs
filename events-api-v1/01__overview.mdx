...
To accomplish customized integrations with PagerDuty, you may be interested in <Link to="/docs/app-integration-development/register-an-app/">building an app or integration with PagerDuty</Link>.
...
<Alert variant="warning">
  ...
  <code>2010-04-15</code> is a version identifier. Do not change the URL to today's date, or any other date.
</Alert>

Head on over to <Link to="/docs/events-api-v1/trigger-events/">Create Event</Link> to see the full details of the event body format.

<Alert variant="info">
  <Alert.Heading>Tip</Alert.Heading>
  Want a simpler way to send events to PagerDuty? The <a href="https://www.pagerduty.com/docs/guides/agent-install-guide/">PagerDuty Agent</a> provides an installable command-line utility for Linux-based systems that handles all the details of event queueing and sending.
</Alert>
...
If a `trigger` event is sent with no `incident_key`, it will always generate a new incident because the key is a <Link to="/docs/rest-api-v2/types/#uuid">UUID</Link>.
