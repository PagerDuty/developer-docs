...
<Alert variant="warning">
  <Alert.Heading>This flow is for server-side apps</Alert.Heading>
  A client_secret should be treated as a password or private_key and not be stored in public code.

  If you're working on an a mobile app or in-browser app, please use the <Link to="/docs/app-integration-development/oauth-2-pkce/">PCKE flow</Link>.
</Alert>

<Link to="/docs/app-integration-development/register-an-app/">Create an app</Link> to get access to OAuth 2.0 credentials.

PagerDuty supports OAuth 2.0’s [Authorization Code Grant](https://tools.ietf.org/html/rfc6749#section-4.1) flow for <Link to="/docs/app-integration-development/register-an-app/">third-party applications</Link> to obtain access tokens from PagerDuty and utilizes the following endpoints:
